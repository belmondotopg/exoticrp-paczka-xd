...
function CreateLootable(index, lootID)
    ...
    Lootables[index][lootID].interact = CreateInteraction({
        ...
        Wait(1500)
        ClearPedTasks(ped)
        TriggerServerEvent("esx_prisons:collectLootable", index, lootID)
    end) 
end

RegisterNetEvent("esx_prisons:lootStatus", function(index, lootID, status) 
    local loot = Lootables[index][lootID]
...
RegisterNetEvent("esx_prisons:setLootables", function(data)
    CleanupLootables()
    Lootables = data
    for k,v in pairs(Lootables) do 
        for i=1, #v do         
