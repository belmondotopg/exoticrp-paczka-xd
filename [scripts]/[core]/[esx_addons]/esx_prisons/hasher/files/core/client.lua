...
function FormatOptions(index, data)
    ...
    if not options or #options < 2 then
        ...
        list[1].icon = "fa-brands fa-fort-awesome"
        list[1].canInteract = function(entity, distance, coords, name, boneId)
    ...
    for i=1, #options do
        ...
        list[i].icon = "fa-brands fa-fort-awesome"
        list[i].canInteract = function(entity, distance, coords, name, boneId)
            if LocalPlayer.state.IsDead then return false end
            if LocalPlayer.state.IsHandcuffed then return false end
            if distance > 2.0 then return false end
            ...
            if (dist < Config.RenderDistance) then 
                ...
                if not Config.UseTarget then
                    if not v.model and Config.Marker and Config.Marker.enabled then
                        wait = 0
                        ESX.DrawMarker(vec3(coords.x, coords.y, coords.z))
                    end
