...
RegisterNetEvent('esx_status:set', function(name, val)
	...
	local statusData = GetStatusData(true)
	TriggerEvent('esx_status:onTick', statusData)
	TriggerServerEvent('esx_status:update', statusData)
end)

RegisterNetEvent('esx_status:add', function(name, val, typ)
	local wasAdded = false
	for i=1, #Status, 1 do
		if Status[i].name == name then
			if typ == "basic" then
				if Status[i].val < (Config.StatusMax / 10) * 3 then
					Status[i].add(val)
					wasAdded = true
				end
			else
