ESX.RegisterServerCallback('esx_tattooshop:requestPlayerTattoos', function(source, cb)
    ...
    if xPlayer then
        MySQL.Async.fetchAll('SELECT tattoos FROM users WHERE identifier = @identifier', {
            ...
            if result[1] and result[1].tattoos and result[1].tattoos ~= '' then
                local tattoos = json.decode(result[1].tattoos)
                cb(tattoos and type(tattoos) == 'table' and tattoos or {})
            else
                cb({})
            end
        end)
...
ESX.RegisterServerCallback('exotic_skin:getPlayerSkin', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
