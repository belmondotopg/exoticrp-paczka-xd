...
window.addEventListener('message', function(event) {
	...
	if (ed.action === "openCarMenu") {
        ...
        if (ed.carData.indicatorState === 1) {
            ...
            topLeft("change", "MDDBDDTopLeftDivIndicatorLeft");
            topLeft("default", "MDDBDDTopLeftDivIndicatorRight");
            ...
            topLeft("default", "MDDBDDTopLeftDivIndicatorLeft");
            topLeft("change", "MDDBDDTopLeftDivIndicatorRight");
...
function clFunc(name1, name2) {
    if (name1 === "door") {
        ...
        // Resetuj animację i ustaw biały kolor
        progressBarText.style.color = '#FFFFFF';
        progressBarText.classList.remove('animating');
        var dots = progressBarText.querySelectorAll('.dot');
        dots.forEach(function(dot) {
            dot.style.animation = 'none';
            dot.style.color = '#FFFFFF';
        });
        
        // Uruchom animację po małym opóźnieniu
        setTimeout(function() {
            progressBarText.classList.add('animating');
            dots.forEach(function(dot) {
                dot.style.animation = 'dotScaleAnimation 1.5s ease-in-out infinite, dotColorAnimation 0.5s ease-in-out 1.5s forwards';
            });
        }, 10);
        
        // Po zakończeniu animacji tekstu i kropek (1.5s tekst + 0.5s kropki = 2s) wykonaj przesiadanie
        setTimeout(function() {
            if (vehData.mySeat) {
