...
local function startTargeting()
    ...
    while state.isActive() do
        ...
        local playerCoords = cache.coords or GetEntityCoords(cache.ped) -- Use cached coords from ox_lib
        
        -- Sprawdź najpierw czy przeciągasz entity, żeby uniknąć niepotrzebnego raycast
        if DoesEntityExist(LocalPlayer.state.DraggedEntity) and LocalPlayer.state.IsDraggingSomeone then
            hit, entityHit, endCoords = true, LocalPlayer.state.DraggedEntity, cache.coords
        else
...
RegisterNUICallback('select', function(data, cb)
    ...
    ---@type OxTargetOption?
    local option = zone and zone.options[data[2]] or (options[data[1]] and options[data[1]][data[2]] or nil)
