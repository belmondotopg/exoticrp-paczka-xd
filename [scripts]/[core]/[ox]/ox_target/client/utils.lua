    ...
    while true do
        ...
        if retval ~= 1 then
            ...
            if DoesEntityExist(LocalPlayer.state.DraggedEntity) and LocalPlayer.state.IsDraggingSomeone then
                return true, LocalPlayer.state.DraggedEntity, endCoords, surfaceNormal, materialHash
            else
...
local hover = vector(255, 136, 0, 255)
local currentZones = {}
local previousZones = {}
local drawZones = {}
local drawN = 0
...
function utils.hasPlayerGotItems(filter, hasAny)
    ...
    if _type == 'string' then
        local itemCount = playerItems[filter]
    ...
    elseif _type == 'table' then
        ...
        if tabletype == 'hash' then
            for name, amount in pairs(filter) do
                local itemCount = playerItems[name]
                local hasItem = type(itemCount) == 'number' and itemCount >= amount
        ...
        elseif tabletype == 'array' then
            for i = 1, #filter do
                local itemCount = playerItems[filter[i]]
                local hasItem = type(itemCount) == 'number' and itemCount > 0
