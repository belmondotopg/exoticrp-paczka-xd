...
local processedZones = lib.context == 'client' and {} --[[@as table<number, boolean>]]
...
CreateThread(function()
    ...
    while true do
        ...
        -- Clear processed zones for this cycle
        table.wipe(processedZones)
            ...
            if contains then
                if not zone.insideZone then
                    ...
                    if zone.onEnter then
                        if not processedZones[zone.id] then
                            enteringZones:push(zone)
                            processedZones[zone.id] = true
                        end
                    end
...
function lib.zones.poly(data)
    -- Check if this zone already exists (prevent duplicates)
    for _, existingZone in pairs(Zones) do
        if existingZone.__type == 'poly' and existingZone.points and data.points then
            if #existingZone.points == #data.points then
                local isDuplicate = true
                for i = 1, #existingZone.points do
                    if #(existingZone.points[i] - data.points[i]) > 0.1 then
                        isDuplicate = false
                        break
                    end
                end
...
function lib.zones.clearAllZones()
...
-- Note: clearAllZones() is available for manual use if needed
-- Automatic clearing was removed as it interfered with other resources
