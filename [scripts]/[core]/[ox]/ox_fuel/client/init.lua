...
local function setVehicleLevel(vehicle, amount, replicated)
    if DoesEntityExist(vehicle) then
        local vehState = Entity(vehicle).state
        local vehAmount = tonumber(amount) or 0

        vehAmount = math.min(math.max(vehAmount, 0), 100)

        SetVehicleFuelLevel(vehicle, vehAmount + 0.0)
        vehState:set('fuel', vehAmount + 0.0, replicated or false)
    end
end
...
local function startDrivingVehicle()
	...
	while cache.seat == -1 do
		if GetIsVehicleEngineRunning(vehicle) then
            ...
            if GetIsVehicleElectric(vehicle) then
                if vehState.fuel < 10 then
                    setVehicleLevel(vehicle,  50.0)
                end
                SetFuelConsumptionRateMultiplier(0.0)
                return
            end

            if config.FuelOff[GetVehicleClass(vehicle)] then
                if vehState.fuel < 10 then
                    setVehicleLevel(vehicle,  50.0)
                end
                SetFuelConsumptionRateMultiplier(0.0)
            ...
            if config.FuelModels[GetEntityModel(vehicle)] then
                if vehState.fuel < 10 then
                    setVehicleLevel(vehicle,  50.0)
                end
                SetFuelConsumptionRateMultiplier(0.0)
			...
			if fuelAmount > 0 then
				...
				if fuelAmount ~= newFuel then
					...
					setVehicleLevel(vehicle, newFuel, fuelTick == 0)
					fuelTick += 1
				end
			end
