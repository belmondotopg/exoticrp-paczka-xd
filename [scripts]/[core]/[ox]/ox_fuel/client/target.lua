	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or esx_hud:progressActive() then
					return false
				end
	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or esx_hud:progressActive() or not DoesVehicleUseFuel(state.lastVehicle) then
					return false
				end
	...
	exports.ox_target:addGlobalVehicle({
		{
			...
			if not state.petrolCan.metadata or not state.petrolCan.metadata.ammo or state.petrolCan.metadata.ammo <= config.durabilityTick then
				return lib.notify({
					type = 'error',
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or esx_hud:progressActive() or not DoesVehicleUseFuel(entity) then
					return false
				end
