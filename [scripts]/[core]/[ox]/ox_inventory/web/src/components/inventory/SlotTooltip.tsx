    ...
    <>
      {!itemData ? (
        <div className="tooltip-wrapper" ref={ref} style={style}>
          ...
          {(description || item.durability || item.metadata) && <Divider />}
        </div>
      ) : (
        <div style={{ ...style }} className="tooltip-wrapper" ref={ref}>
          ...
          {(description || item.durability) && <Divider />}
          {description && (
          ...
          {inventoryType !== 'crafting' ? (
            ...
            (item.metadata?.components && item.metadata.components.length) ||
            item.metadata?.weapontint ||
            additionalMetadata.some(data => item.metadata && item.metadata[data.metadata])) && (
              <div>
                {item.durability !== undefined && (
                  <p>
