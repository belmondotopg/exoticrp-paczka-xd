...
local anims = {}
anims[`GROUP_MELEE`] = { 'melee@holster', 'unholster', 400, 'melee@holster', 'holster', 450 }
anims[`GROUP_PISTOL`] = { 'rcmjosh4', 'josh_leadout_cop2', 425, 'rcmjosh4', 'josh_leadout_cop2', 475 }
...
function Weapon.Equip(item, data)
	...
	if client.weaponanims then
		...
		if anims[GetWeapontypeGroup(data.hash)] == nil and data.hash ~= 615608432 and data.hash ~= -73270376 and data.hash ~= 883325847 and data.hash ~= -656458692 then
			Citizen.Wait(100)
			TriggerServerEvent('ox_inventory:komunikat', 'Wyciąga broń długą')
		end

		if (GetWeapontypeModel(data.hash) == 32653987 or GetWeapontypeModel(data.hash) == -1713889927 or GetWeapontypeModel(data.hash) == 1653948529) and data.hash ~= 615608432 or data.hash == -656458692 then
			Citizen.Wait(100)
			TriggerServerEvent('ox_inventory:komunikat', 'Wyciąga broń białą')
			anim = nil
		end

	sleep = anim and anim[3] or 1200

	local count = exports.ox_inventory:Search('count', 'kabura')
...
function Weapon.Disarm(currentWeapon, noAnim)
	if currentWeapon?.timer then
		...
		if client.weaponanims and not noAnim then
			...
			if anims[GetWeapontypeGroup(currentWeapon.hash)] == nil and currentWeapon.hash ~= 615608432 and currentWeapon.hash ~= -73270376 and currentWeapon.hash ~= 883325847 and currentWeapon.hash ~= -656458692 then
				Citizen.Wait(100)
				TriggerServerEvent('ox_inventory:komunikat', 'Chowa broń długą')
			end
	
			if (GetWeapontypeModel(currentWeapon.hash) == 32653987 or GetWeapontypeModel(currentWeapon.hash) == -1713889927 or GetWeapontypeModel(currentWeapon.hash) == 1653948529) and currentWeapon.hash ~= 615608432 or currentWeapon.hash == -656458692 then
				Citizen.Wait(100)
				TriggerServerEvent('ox_inventory:komunikat', 'Chowa broń białą')
				anim = nil
			end
