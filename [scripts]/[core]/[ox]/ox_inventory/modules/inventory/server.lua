...
local function SendLog(webhook, name, message)
	local embeds = {
		{
	...
	if webhook ~= nil then 
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({ username = 'ExoticRP', avatar_url = 'https://i.ibb.co/kVsZnt71/logo.png', embeds = embeds}), { ['Content-Type'] = 'application/json' })
	end
end
...
function Inventory.SwapSlots(fromInventory, toInventory, slot1, slot2)
	...
	if fromSlot then 
		fromSlot.slot = slot2
		if fromSlot.name and Items(fromSlot.name)?.weapon then
			if not fromSlot.metadata then fromSlot.metadata = {} end
			if not fromSlot.metadata.durability or type(fromSlot.metadata.durability) ~= 'number' then
				fromSlot.metadata.durability = 100
			end
		end
	end
	if toSlot then 
		toSlot.slot = slot1
		if toSlot.name and Items(toSlot.name)?.weapon then
			if not toSlot.metadata then toSlot.metadata = {} end
			if not toSlot.metadata.durability or type(toSlot.metadata.durability) ~= 'number' then
				toSlot.metadata.durability = 100
			end
		end
	end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if toSlotType == 'number' then
		...
		if invokingResource then
			lib.logger(inv.owner, 'addItem', ('"%s" dodano %sx %s do "%s"'):format(invokingResource, count, item.name, inv.label))
		end
		if inv.player then
			esx_core:SendLog(inv.id, "Dodano przedmiot", "Dodano przedmiot: "..item.name.. " " ..count .. "x", 'additem')
		end
	...
	elseif toSlotType == 'table' then
		...
		if invokingResource then
			lib.logger(inv.owner, 'addItem', ('"%s" dodano %sx %s do "%s"'):format(invokingResource, added, item.name, inv.label))
		end
		if inv.player then
			esx_core:SendLog(inv.id, "Dodano przedmiot", "Dodano przedmiot: "..item.name.. " " ..added .. "x", 'additem')
		end
...
local function dropItem(source, playerInventory, fromData, data)
	...
	local InventoryIdentifiers = ESX.ExtractIdentifiers(playerInventory.id)
	
	esx_core:SendLog(playerInventory.id, "DropItem", "Wyrzucił na ziemie przedmiot: "..toData.name.. " " ..data.count .. "x\n\nEkwipunek otrzymujący\nWłaściciel: "..playerInventory.label.."\nDropID: "..dropId.."\nID: "..playerInventory.id.."\nLicencja: "..InventoryIdentifiers.license.."\nDiscord: <@"..InventoryIdentifiers.discord..">", 'drop')
	
	if toData.name == 'money' and data.count > 200000 then
		esx_core:SendLog(playerInventory.id, "SuspiciousDrop", "Wyrzucił na ziemie przedmiot: "..toData.name.. " " ..data.count .. "x\n\nEkwipunek otrzymujący\nWłaściciel: "..playerInventory.label.."\nDropID: "..dropId.."\nID: "..playerInventory.id.."\nLicencja: "..InventoryIdentifiers.license.."\nDiscord: <@"..InventoryIdentifiers.discord..">", 'suspicious')
	end
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if not sameInventory then
		local contrabandStashIds = {'policeschowek1', 'sheriffschowek1', 'policeschowek2', 'sheriffschowek2'}
		local fromStashId = fromInventory.id
		local toStashId = toInventory.id
		local isContrabandStash = false
		
		for _, stashId in ipairs(contrabandStashIds) do
			if fromStashId == stashId or toStashId == stashId then
				isContrabandStash = true
				break
			end
		end
		
		if isContrabandStash then
			if fromInventory.type == 'stash' and toInventory.type == 'player' then
				...
				if xPlayer and xPlayer.job then
					...
					if (jobName == 'police' or jobName == 'sheriff') and jobGrade >= 10 then
						-- Pozwól na branie
					else
						xPlayer.showNotification('Nie masz wystarczającej rangi, aby wyjmować przedmioty z tego schowka!', 'error')
						return false
					end
				else
					if xPlayer then
						xPlayer.showNotification('Nie masz uprawnień do tego schowka!', 'error')
					end
					return false
				end
			end
	...
	local function GetInventoryType(inventory)
		local typeMap = {
			['glovebox'] = 'Schowek',
			['container'] = 'Torba',
			['evidence'] = 'Dowód',
		...
		return typeMap[inventory.type] or ''
	end

	local function LogOrganizationActivity(src, xPlayer, fromInventory, toInventory, fromData, count)
		local fromInventoryId = tostring(fromInventory.id or '')
		local toInventoryId = tostring(toInventory.id or '')
		
		if fromInventoryId:find("^organisation_") or toInventoryId:find("^organisation_") then
			local Identifiers = ESX.ExtractIdentifiers(src)
			local message = string.format(
				"\nID: %d | %s | <@!%s> | %s\nWyjął z: %s\nPrzedmiot: %s w ilości: %dx\nWłożył do: %s\nPrzedmiot: %s w ilości: %dx",
				xPlayer.source, GetPlayerName(src), Identifiers.discord or "Brak", xPlayer.identifier,
				fromInventory.label, fromData.name, count, toInventory.label, fromData.name, count
			)

			esx_core:SendLog(src, "InventoryOrg", message, 'inventory-szafki-org')
		end
	end

	local function LogFractionActivity(src, xPlayer, fromInventory, toInventory, fromData, count)
		if not xPlayer.job then return end
		
		local fromWhere = tostring(fromInventory.label or fromInventory.id)
		local toWhere = tostring(toInventory.label or toInventory.id)
		...
		if (xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' or xPlayer.job.name == 'ambulance') then
			...
			local fromIsPolice = fromWhere:find("[LSPD/LSSD]")
			local toIsPolice = toWhere:find("[LSPD/LSSD]")
			local fromIsEMS = fromWhere:find("EMS")
			local toIsEMS = toWhere:find("EMS")
			
			local shouldLog = false
			if isPolice and ((fromIsPolice and toIsPolice) or (fromIsPolice and fromInventory.type == 'player') or (toIsPolice and toInventory.type == 'player')) then
				shouldLog = true
			elseif isAmbulance and ((fromIsEMS and toIsEMS) or (fromIsEMS and fromInventory.type == 'player') or (toIsEMS and toInventory.type == 'player')) then
				shouldLog = true
			end
			
			if shouldLog then
				local fractionResult = MySQL.query.await("SELECT webhook FROM webhook_fraction WHERE name = ?", {xPlayer.job.name})
				
				local Identifiers = ESX.ExtractIdentifiers(src)
				local jobLabel = ESX.GetJobLabelFromName(xPlayer.job.name)
				local message = string.format(
					"\nID: %d | %s | <@!%s> | %s\nWyjął z: %s\nPrzedmiot: %s w ilości: %dx\nWłożył do: %s\nPrzedmiot: %s w ilości: %dx",
					xPlayer.source, GetPlayerName(src), Identifiers.discord or "Brak", xPlayer.identifier,
					fromInventory.label, fromData.name, count, toInventory.label, fromData.name, count
				...
				if fractionResult and fractionResult[1] and fractionResult[1].webhook then
					local webhook = tostring(fractionResult[1].webhook)
					SendLog(webhook, string.format("Szafka frakcji: %s | %s", jobLabel, toInventory.label), message)
				end
				esx_core:SendLog(src, "InventoryFraction", message, 'inventory-szafki-fraction')
			end
		end
	end
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromInventory.type == 'player' and not sameInventory and (fromData.name == 'money' or fromData.name == 'black_money') then
			if fromOtherPlayer or toOtherPlayer then
				return false, 'cannot_move_money'
			end
		end

		if fromInventory.type == 'player' and not sameInventory and string.find(fromData.name, "rod_") then
			if fromOtherPlayer or toOtherPlayer then
				return false, 'cannot_move_rod'
			end
		end
		...
		if fromData then
			...
			if toInventory.type == 'stash' or toInventory.type == 'trunk' or toInventory.type == 'glovebox' then
				if fromData.name and (fromData.name == 'cash' or fromData.name == 'money') then
					return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = 'Nie możesz przechowywać czystej gotówki w schowkach!' })
				end
			end
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						if xPlayer then
							...
							local fromDataName = fromData.name
							local fromWhere = tostring(fromInventory.label or fromInventory.id)
							local toWhere = tostring(toInventory.label or toInventory.id)
							local fromType = GetInventoryType(fromInventory)
							local toType = GetInventoryType(toInventory)
							local plType = fromType ~= '' and fromType or toType

							LogOrganizationActivity(src, xPlayer, fromInventory, toInventory, fromData, count)
							LogFractionActivity(src, xPlayer, fromInventory, toInventory, fromData, count)

							local fromOwner = fromInventory.owner == false or fromInventory.owner == true
							local toOwner = toInventory.owner == false or toInventory.owner == true
							local bothOwners = toInventory.owner and fromInventory.owner

							if fromOwner or toOwner or bothOwners then
								local FromIdentifiers = fromOwner and ESX.ExtractIdentifiers(toInventory.id) or (bothOwners and ESX.ExtractIdentifiers(fromInventory.id) or nil)
								local ToIdentifiers = toOwner and ESX.ExtractIdentifiers(fromInventory.id) or (bothOwners and ESX.ExtractIdentifiers(toInventory.id) or nil)

								local logMessage
								if bothOwners and FromIdentifiers and ToIdentifiers then
									logMessage = string.format(
										"Przeniesiono przedmiot: %s %dx\n\nEkwipunek przenoszący\nWłaściciel: %s\nID: %s\nLicencja: %s\nDiscord: <@!%s>\n\nEkwipunek otrzymujący\nWłaściciel: %s\nID: %s\nLicencja: %s\nDiscord: <@!%s>",
										fromDataName, count,
										fromInventory.label or fromInventory.id, fromInventory.id, FromIdentifiers.license or "Brak", FromIdentifiers.discord or "Brak",
										toInventory.label or toInventory.id, toInventory.id, ToIdentifiers.license or "Brak", ToIdentifiers.discord or "Brak"
									)
								elseif fromOwner and FromIdentifiers then
									logMessage = string.format(
										"Przeniesiono przedmiot: %s %dx\n\nEkwipunek przenoszący\nWłaściciel: %s\nTyp: %s\nID: %s\n\nEkwipunek otrzymujący\nWłaściciel: %s\nID: %s\nLicencja: %s\nDiscord: <@%s>",
										fromDataName, count,
										fromInventory.label or fromInventory.id, plType, fromInventory.id,
										toInventory.label or toInventory.id, toInventory.id, FromIdentifiers.license or "Brak", FromIdentifiers.discord or "Brak"
									)
								elseif toOwner and ToIdentifiers then
									logMessage = string.format(
										"Przeniesiono przedmiot: %s %dx\n\nEkwipunek przenoszący\nWłaściciel: %s\nID: %s\nLicencja: %s\nDiscord: <@%s>\n\nEkwipunek otrzymujący\nWłaściciel: %s\nID: %s\nTyp: %s",
										fromDataName, count,
										fromInventory.label or fromInventory.id, fromInventory.id, ToIdentifiers.license or "Brak", ToIdentifiers.discord or "Brak",
										toInventory.label or toInventory.id, toInventory.id, plType
									)
								end

								if logMessage then
									esx_core:SendLog(playerInventory.id, "SwapItems", logMessage, 'swap')
								end
							end
						end
					else
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						local fromLabel = fromInventory.owner and fromInventory.label or fromInventory.id
						local toLabel = toInventory.owner and toInventory.label or toInventory.id
						local logMessage = string.format('%dx %s przeniósł z szafki / ekwipunku / gracza "%s" do "%s"', data.count, fromData.name, fromLabel, toLabel)
						
						if toLabel == 'Dom' or fromLabel == 'Dom' then
							esx_core:SendLog(playerInventory.id, "MoveToApartment", logMessage, 'apartamenty-szafka')
						elseif (type(fromInventory.id) == 'string' and fromInventory.id:find('trunk')) or (type(toInventory.id) == 'string' and toInventory.id:find('trunk')) then
							esx_core:SendLog(playerInventory.id, "MoveToTrunk", logMessage, 'bagaznik-szafka')
						else
							esx_core:SendLog(playerInventory.id, "MoveToOther", logMessage, 'other')
						end
					end
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						local xPlayer = ESX.GetPlayerFromId(src)
						if xPlayer then
							LogOrganizationActivity(src, xPlayer, fromInventory, toInventory, fromData, data.count)
							LogFractionActivity(src, xPlayer, fromInventory, toInventory, fromData, data.count)
						end
						...
						if toLabel == 'Dom' or fromLabel == 'Dom' then
							esx_core:SendLog(playerInventory.id, "MoveToApartmentInventory", logMessage, 'apartamenty-szafka')
						elseif (type(fromInventory.id) == 'string' and fromInventory.id:find('trunk')) or (type(toInventory.id) == 'string' and toInventory.id:find('trunk')) then
							esx_core:SendLog(playerInventory.id, "MoveToTrunkInventory", logMessage, 'bagaznik-szafka')
						else
							esx_core:SendLog(playerInventory.id, "MoveToOtherInventory", logMessage, 'other')
						end
					end
			...
			if fromInventory.id ~= nil then
				fromPlayer = ESX.GetPlayerFromId(fromInventory.id)
			end

			if fromPlayer then
				local x = toInventory.items[data.toSlot] or { slot = data.toSlot }
				if x.name == 'gps' and (fromPlayer.job.name == "police" or fromPlayer.job.name == "ambulance") and fromInventory.id ~= toInventory.id then
					local badge = fromPlayer.badge or ""
					local firstName = fromPlayer.get('firstName') or ""
					local lastName = fromPlayer.get('lastName') or ""
					local fullName = (firstName ~= "" and lastName ~= "") and (firstName .. " " .. lastName) or fromPlayer.getName()
					local data = {
						name = (badge ~= "" and '['..badge..'] ' or '')..fullName..' - '..ESX.GetGradeLabel(fromPlayer.job.name, fromPlayer.job.grade),
						firstName = firstName,
						lastName = lastName,
						coords = GetEntityCoords(GetPlayerPed(fromInventory.id))
					}
					...
					for k,v in pairs(GetPlayers) do
						if sendOne[v.id] then return end
						sendOne[v.id] = true
						if v.job == 'police' or v.job == 'ambulance' then		
							TriggerClientEvent('esx_police:removedGPS', v.id, data)
						end
					end
				end
			end
		...
		if TriggerEventHooks('swapItems', {
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
					local FromIdentifiers = ESX.ExtractIdentifiers(fromInventory.id)
					local ToIdentifiers = ESX.ExtractIdentifiers(toInventory.id)
					esx_core:SendLog(fromInventory.id, "Przekazanie przedmiotów", "Przekazano przedmiot: "..data.name.. " " ..count.."x\n\nEkwipunek przekazujący\nWłaściciel: "..fromInventory.label.."\nID: "..fromInventory.id.."\nLicencja: "..FromIdentifiers.license.."\nDiscord: <@"..FromIdentifiers.discord..">\n\nEkwipunek otrzymujący\nWłaściciel: "..toInventory.label.."\nID: "..toInventory.id.."\nLicencja: "..ToIdentifiers.license.."\nDiscord: <@"..ToIdentifiers.discord..">", 'swap')
					return
				else
...
lib.callback.register('ox_inventory:hasPIN', function(source, plate)
	if not plate then return end

	local result = MySQL.single.await('SELECT `pin` FROM `glovebox_pins` WHERE `plate` = ?', {plate})

	if result then
		if tonumber(result.pin) ~= nil then
			return true
		end
	end
...
lib.callback.register('ox_inventory:checkAccessPIN', function(source, pin, plate)
	...
	if result then
		if tonumber(result.pin) == tonumber(pin) then
			return true
		end
	end
...
lib.callback.register('ox_inventory:setPIN', function(source, pin, plate)
	if not pin then return end
	if not plate then return end

	local xPlayer = ESX.GetPlayerFromId(source)
	local result = MySQL.single.await('SELECT vehicle FROM owned_vehicles WHERE (owner = @owner) or (co_owner = @owner) or (co_owner2 = @owner) or (co_owner3 = @owner) AND plate = @plate', { ['@owner'] = xPlayer.identifier, ['@plate'] = plate })
	...
	if result then
		...
		if result then
			MySQL.update.await(
				'UPDATE `glovebox_pins` SET pin = @pin WHERE plate = @plate',
				{
					['@plate'] = plate,
					['@pin'] = tonumber(pin)
				}
			...
			MySQL.update('INSERT INTO `glovebox_pins` (plate, pin) VALUES (?, ?)', {plate, pin})
			return true
		end
	else
		return false
