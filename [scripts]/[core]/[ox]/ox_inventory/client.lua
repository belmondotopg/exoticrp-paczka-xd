...
local function canOpenTarget(ped)
	...
	or LocalPlayer.state.InTrunk
	or LocalPlayer.state.IsHandcuffed
end
...
lib.callback.register('ox_inventory:usingItem', function(data, noAnim)
	...
	if item and usingItem then
		...
		local success = (not item.usetime or esx_hud:progressBar({
...
local function registerCommands()
	local function openGlovebox(vehicle)
		...
		lib.callback('ox_inventory:checkIfVehicleIsOwned', false, function(owned)
            if owned then
				local options = {}
				local menu = nil

				options[0] = {
					title = 'Ustaw / zmień PIN do schowka',
					onSelect = function()
						lib.hideMenu(menu)

						local input = lib.inputDialog('Nowy PIN', {
							{type = 'number', label = 'Schowek', description = 'Nowy PIN do schowka', required = true, min = 0, max = 9999},
						})

						if input then
							if input[1] then
								lib.callback("ox_inventory:setPIN", false, function(changed)
									if changed then
										ESX.ShowNotification('PIN ['..input[1]..'] został ustawiony pomyślnie!')
									else
										ESX.ShowNotification('Wystąpił błąd podczas ustawiania kodu PIN!')
									end
								end, input[1], plate)
							end
						end
				...
				menu = lib.registerContext({
					id = 'ox_inventory:openOwnerContext',
					title = 'Zarządzaj',
					options = options
				})
				...
				lib.showContext('ox_inventory:openOwnerContext')
            else
				if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'sheriff' then
					...
					lib.callback("ox_inventory:hasPIN", false, function(has)
						if has then
							local options = {}
							local menu = nil
	
							options[0] = {
								title = 'Wpisz PIN aby otworzyć schowek',
								onSelect = function()
									lib.hideMenu(menu)
	
									local input = lib.inputDialog('Schowek - PIN', {
										{type = 'number', label = 'Schowek', description = 'Podaj PIN aby otworzyć schowek', required = true, min = 0, max = 9999},
									})
			
									if input then
										if input[1] then
											lib.callback("ox_inventory:checkAccessPIN", false, function(correct)
												if correct then
													ESX.ShowNotification('Wprowadzono prawidłowy kod PIN!')
													...
													ESX.ShowNotification('Wprowadzono niepoprawny kod PIN!')
												end
											end, input[1], plate)
										end
									end
							...
							menu = lib.registerContext({
								id = 'ox_inventory:openContext',
								title = 'Zarządzaj',
								options = options
							})
	
							lib.showContext('ox_inventory:openContext')
						else
							local isOpen = client.openInventory('glovebox', { id = 'glove'..plate, netid = NetworkGetNetworkIdFromEntity(vehicle) })
	...
	local primary = lib.addKeybind({
		...
		onPressed = function()
			...
			if closest and closest.currentDistance < 2.5 and (not closest.instance or closest.instance == currentInstance) then
        ...
        if item then
            ...
            if count < 0 then
                if shared.framework == 'esx' then
					...
					TriggerServerEvent('esx:onRemoveInventoryItem', cache.serverId, itemName, itemCount)
                end
            ...
            elseif count > 0 then
                if shared.framework == 'esx' then
					...
					TriggerServerEvent('esx:onAddInventoryItem', cache.serverId, itemName, itemCount)
                end
...
RegisterNUICallback('giveItem', function(data, cb)
	...
	if client.giveplayerlist then
		...
		local options = {}
		options[0] = {
			title = 'Wybierz osobę której chcesz przekazać przedmiot',
			disabled = true,
		}
		...
		for i = 1, #nearbyPlayers do
			...
			if isGiveTargetValid(option.ped, option.coords) then
				local clientId = option.id
				local serverId = GetPlayerServerId(clientId)
				local playerName = GetPlayerName(clientId)

				-- Próba pobrania imienia RP ze StateBag
				local playerState = Entity(option.ped).state
				if playerState and playerState.identityName then
					playerName = playerState.identityName
				end

				options[serverId] = {
					title = ('[%s] %s'):format(serverId, playerName or "Nieznany"),
					onSelect = function()
						local dict, anim = "mp_common", "givetake1_a"
						ESX.Streaming.RequestAnimDict(dict)
						
						TaskPlayAnim(playerPed, dict, anim, 8.0, -8.0, 2000, 49, 0.0, false, false, false)
						RemoveAnimDict(dict)

						Wait(2000)

						return giveItemToTarget(serverId, data.slot, data.count)
					end
				}
				n = n + 1
			end
...
RegisterNetEvent('ox_inventory:changeColor', function(color)
    if color then
        SendNUIMessage({ action = 'changeColor', data = color })
    end
end)