...
local function noclipThread()
    while noclip do
        ...
        SetEntityCoordsNoOffset(ped, noclip_pos.x, noclip_pos.y, noclip_pos.z, false, false, true)
        ...
        if IsControlPressed(1, 8) then
            noclip_pos = GetOffsetFromEntityInWorldCoords(ped, 0.0, -1.0, 0.0)
        end
        ...
        if IsControlPressed(1, 27) then
            noclip_pos = GetOffsetFromEntityInWorldCoords(ped, 0.0, 0.0, 1.0)
        end
    ...
    -- Reset entity properties when no-clip is disabled
    local ped = ESX.PlayerData.ped
...
RegisterNetEvent("esx:noclip", function()
    ESX.TriggerServerCallback("esx:isUserAdmin", function(admin)
        ...
        if noclip then
            ...
            -- Reset entity properties when disabling no-clip
            local ped = ESX.PlayerData.ped
            ResetEntityAlpha(ped)
            SetEntityCollision(ped, true, true)
            FreezeEntityPosition(ped, false)
...
ESX.RegisterClientCallback("esx:IsVehicleExist", function(cb, model)
    cb(ESX.IsVehicleExistClient(model))
end)
...
-- Obsługa komendy /save - pobranie właściwości pojazdu i wysłanie do serwera
RegisterNetEvent('es_extended:getPlayerFromCar', function(vehicleType)
	local ped = ESX.PlayerData.ped
	local vehicle = GetVehiclePedIsIn(ped, false)
	...
	-- Pobierz właściwości pojazdu
	local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)
	
	if not vehicleProps then
		ESX.ShowNotification("Nie udało się pobrać właściwości pojazdu!", "error")
		return
	end
	
	-- Wyślij właściwości pojazdu do serwera
	TriggerServerEvent('es_extended:saveVehicleToDatabase', vehicleProps, vehicleType)
end)
