...
Citizen.CreateThread(function()
	...
	while true do 
		...
		if backupInfos == nil then 
			...
			if backupData and backupData.lastBackup then
				lastBackupTime = backupData.lastBackup
			else
				lastBackupTime = 0
			end
		end
		if lastBackupTime and (GetConvarInt("ea_backupFrequency", 72) ~= 0) and (lastBackupTime+(GetConvarInt("ea_backupFrequency", 72)*3600) < os.time()) then
			createBackup()
		end
		Wait(120000)
	end
...
function createBackup()
	...
	return backupData.backups[#backupData.backups].id, backupTime
end
