...
function getAllPlayerIdentifiers(playerId) --Gets all info that could identify a player
	...
	if GetConvar("ea_useTokenIdentifiers", "true") == "true" then
		if not GetNumPlayerTokens or not GetPlayerToken then
			PrintDebugMessage(
				"Server Version is below artifact 3335, disabling Token identifiers, please consider updating your FXServer!",
				1)
	...
	if IsPlayerAdmin(source) then
		...
		for i, player in pairs(players) do
			...
			local pData = {
				...
				discord =
					cachedPlayer.discord,
				contributor = Contributors[cachedPlayer.discord],
				developer = cachedPlayer.discord ==
					"178889658128793600"
			}
...
Citizen.CreateThread(function()
	if not CachedPlayers or GetVersion() == nil then
		print("^7--------------------------------------------------------------")
		print(
			"^1EasyAdmin self-test failed! Your EasyAdmin **will not work**, likely you edited some files and broke EasyAdmin in the progress, please reinstall EasyAdmin.")
		print("^7--------------------------------------------------------------")
		return
	end
	...
	RegisterServerEvent('EasyAdmin:amiadmin', function()
		...
		if CachedPlayers[source].lastPermRequest and CachedPlayers[source].lastPermRequest + 10 > os.time() then
			PrintDebugMessage(
				getName(source) ..
				" hit Permission Check Ratelimit! " .. CachedPlayers[source].lastPermRequest + 10 - os.time() ..
				" seconds left.", 3)
			return
		end
		...
		for perm, val in pairs(permissions) do
			...
			if perm == "player.screenshot" and GetResourceState("screenshot-basic") == "missing" then
				thisPerm = false
			end
			...
			perms[perm] = thisPerm
			PrintDebugMessage("Processed Perm " .. perm ..
				" for " .. getName(source, true) .. ", result: " .. tostring(thisPerm), 3)
		end
		...
		TriggerClientEvent('chat:addSuggestion', source, '/easyadmin', "EasyAdmin Menu",
			{ { name = "report or player id", help = "[Optional] Report or Player ID" } })
		TriggerClientEvent('chat:addSuggestion', source, '/ea', "EasyAdmin Menu",
			{ { name = "report or player id", help = "[Optional] Report or Player ID" } })

		if GetConvar("ea_enableReportCommand", "true") == "true" then
			TriggerClientEvent('chat:addSuggestion', source, '/' .. GetConvar("ea_reportCommandName", "report"),
				"Report player", { { name = 'player', help = "player name / id" }, { name = 'reason', help = "Reason" } })
		end

		if GetConvar("ea_enableCallAdminCommand", "true") == "true" then
			TriggerClientEvent('chat:addSuggestion', source, '/' .. GetConvar("ea_callAdminCommandName", "calladmin"),
				"Call Admin", { { name = 'reason', help = "Reason" } })
		end
		...
		if os.date("%d/%m") == "22/08" then
			local age = tonumber(os.date("%Y")) - 2017
			local ordinal = "th"
			last_digit = age % 10
			if last_digit == 1 and age ~= 11 then
				ordinal = 'st'
			elseif last_digit == 2 and age ~= 12 then
				ordinal = 'nd'
			elseif last_digit == 3 and age ~= 13 then
				ordinal =
				'rd'
			end
			TriggerClientEvent("EasyAdmin:SetSetting", source, "alternativeTitle",
				"~b~Today is EasyAdmin's " .. age .. "" .. ordinal .. " birthday! :)")
	...
	RegisterServerEvent("EasyAdmin:kickPlayer", function(playerId, reason)
		if DoesPlayerHavePermission(source, "player.kick") and CheckAdminCooldown(source, "kick") and not CachedPlayers[playerId].immune then
			...
			SendWebhookMessage(moderationNotification,
				string.format(GetLocalisedText("adminkickedplayer"), getName(source, false, true),
					getName(playerId, true, true), reason), "kick", 16711680)
			...
			exports.esx_core:SendLog(playerId, "Wyrzucenie gracza", "Wyrzucony z serwera przez ".. GetPlayerName(source), 'admin-kick')
	...
	RegisterServerEvent("EasyAdmin:requestSpectate", function(playerId)
		if DoesPlayerHavePermission(source, "player.spectate") and CheckAdminCooldown(source, "spectate") then
			...
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('spectatedplayer'), getName(source, false, true),
					getName(playerId, true, true)), "spectate", 16777214)
		end
	end)

	RegisterServerEvent("EasyAdmin:revivePlayer", function(playerId)
		if DoesPlayerHavePermission(source, "player.revive") and CheckAdminCooldown(source, "revive") then
			SetAdminCooldown(source, "revive")
			local tgtPed = GetPlayerPed(playerId)
			if tgtPed == 0 then
				TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("playernotfound"))
				return
			...
			TriggerClientEvent('esx_ambulance:onTargetRevive', playerId)
		end
	end)

	RegisterNetEvent("EasyAdmin:callPlayer", function(playerId)
		if DoesPlayerHavePermission(source, "player.call") and CheckAdminCooldown(source, "call") then
			SetAdminCooldown(source, "call")
			local tgtPed = GetPlayerPed(playerId)
			if tgtPed == 0 then
				TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("playernotfound"))
				return
			...
			TriggerClientEvent('EasyAdmin:callPlayer', playerId)
		end
	end)

	RegisterServerEvent("EasyAdmin:healPlayer", function(playerId)
		if DoesPlayerHavePermission(source, "player.heal") and CheckAdminCooldown(source, "heal") then
			SetAdminCooldown(source, "heal")
			local tgtPed = GetPlayerPed(playerId)
			if tgtPed == 0 then
				TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("playernotfound"))
				return
	...
	RegisterServerEvent("EasyAdmin:requestCleanup", function(type, radius, deep)
		...
		if DoesPlayerHavePermission(source, "server.cleanup." .. type) then
			...
			TriggerClientEvent("EasyAdmin:showNotification", source,
				string.format(GetLocalisedText("finishedcleaning"), GetLocalisedText(type)))
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('admincleanedup'), getName(source, false, true), type, radius), "cleanup",
				16777214)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:SetGameType", function(text)
		if DoesPlayerHavePermission(source, "server.convars") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminchangedconvar'), getName(source, false, true), "gametype", text),
				"settings", 16777214)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:SetMapName", function(text)
		if DoesPlayerHavePermission(source, "server.convars") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminchangedconvar'), getName(source, false, true), "mapname", text),
				"settings", 16777214)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:StartResource", function(text)
		if DoesPlayerHavePermission(source, "server.resources.start") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminstartedresource'), getName(source, false, true), text), "settings",
				65280)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:StopResource", function(text)
		if DoesPlayerHavePermission(source, "server.resources.stop") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminstoppedresource'), getName(source, false, true), text), "settings",
				16711680)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:SetConvar", function(convarname, convarvalue)
		if DoesPlayerHavePermission(source, "server.convars") then
			PrintDebugMessage(
				"Player " .. getName(source, true) .. " set convar " .. convarname .. " to " .. convarvalue,
				3)
			SetConvar(convarname, convarvalue)
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminchangedconvar'), getName(source, false, true), convarname,
					convarvalue), "settings", 16777214)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:Announce", function(text)
		if DoesPlayerHavePermission(source, "server.announce") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText('adminannouncement'), getName(source, false, true), text), "settings",
				16777214)
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:TeleportPlayerToCoords", function(playerId, tgtCoords)
		...
		if DoesPlayerHavePermission(source, "player.teleport.single") and CheckAdminCooldown(source, "teleport") then
			...
			PrintDebugMessage(
				"Player " ..
				getName(source, true) .. " requsted teleport to " .. tgtCoords.x .. ", " .. tgtCoords.y .. ", " ..
				tgtCoords.z, 3)
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			...
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText("teleportedtoplayer"), playerName, getName(source, false, true)),
				"teleport", 16777214)
	...
	RegisterServerEvent("EasyAdmin:TeleportAdminToPlayer", function(id)
		...
		if not CachedPlayers[id].dropped and DoesPlayerHavePermission(source, "player.teleport.single") and CheckAdminCooldown(source, "teleport") then
			...
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText("teleportedtoplayer"), getName(source, false, true),
					getName(id, true, true)), "teleport", 16777214)
	...
	RegisterServerEvent("EasyAdmin:TeleportPlayerToPlayer", function(id, targetId)
		local source = source
		if not CachedPlayers[id].dropped and DoesPlayerHavePermission(source, "player.teleport.single") and CheckAdminCooldown(source, "teleport") then
			SetAdminCooldown(source, "teleport")
			local tgtPed = GetPlayerPed(id)
			...
			local targetCoords = GetEntityCoords(tgtPed)
			if targetCoords.x == 0.00 and targetCoords.y == 0.00 then
				-- loaded in but still spawning
				TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("playernotfound"))
				return
			end
			TriggerClientEvent('EasyAdmin:TeleportRequest', id, targetId, targetCoords)
		else
			TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("playernotfound"))
			PrintDebugMessage('EASYADMIN FAILED TO TELEPORT' .. source .. ' TO ID: ' .. id, 2)
		end
	...
	RegisterServerEvent("EasyAdmin:SlapPlayer", function(playerId, slapAmount)
		if DoesPlayerHavePermission(source, "player.slap") and CheckAdminCooldown(source, "slap") and slapPlayer(playerId, slapAmount) then
			...
			PrintDebugMessage(
				"Player " .. getName(source, true) .. " slapped " .. getName(playerId, true) .. " for " .. slapAmount ..
				" HP", 3)
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,
				string.format(GetLocalisedText("adminslappedplayer"), getName(source, false, true),
					getName(playerId, true, true), slapAmount), "slap", 16777214)
		elseif CachedPlayers[playerId].immune then
			TriggerClientEvent("EasyAdmin:showNotification", source, GetLocalisedText("adminimmune"))
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:FreezePlayer", function(playerId, toggle)
		if DoesPlayerHavePermission(source, "player.freeze") and not CachedPlayers[playerId].immune and CheckAdminCooldown(source, "freeze") then
			...
			if toggle then
				...
				SendWebhookMessage(preferredWebhook,
					string.format(GetLocalisedText("adminfrozeplayer"), getName(source, false, true),
						getName(playerId, true, true)), "freeze", 16777214)
				PrintDebugMessage("Player " .. getName(source, true) .. " froze " .. getName(playerId, true), 3)
			else
				SendWebhookMessage(preferredWebhook,
					string.format(GetLocalisedText("adminunfrozeplayer"), getName(source, false, true),
						getName(playerId, true, true)), "freeze", 16777214)
	...
	RegisterServerEvent("EasyAdmin:TakeScreenshot", function(playerId)
		...
		if DoesPlayerHavePermission(source, "player.screenshot") and CheckAdminCooldown(source, "screenshot") then
			...
			thistemporaryevent = RegisterServerEvent("EasyAdmin:TookScreenshot", function(result)
				...
				SendWebhookMessage(moderationNotification,
					string.format(GetLocalisedText("admintookscreenshot"), invokingResource or getName(src),
						getName(playerId, true, true), res), "screenshot", 16777214, "Screenshot Captured", res)
				TriggerClientEvent('chat:addMessage', src,
					{ template = '<img src="{0}" style="max-width: 400px;" />', args = { res } })
				TriggerClientEvent("chat:addMessage", src,
					{ args = { "EasyAdmin", string.format(GetLocalisedText("screenshotlink"), res) } })
				PrintDebugMessage(
					"Screenshot for Player " .. getName(playerId, true) .. " done, " ..
					res .. " requsted by" .. getName(src, true), 3)
				scrinprogress = false
		...
		if DoesPlayerHavePermission(src, "player.mute") and not CachedPlayers[playerId].immune and CheckAdminCooldown(source, "mute") then
			...
			if muted then
				if MutedPlayers[playerId] then
					TriggerClientEvent("EasyAdmin:showNotification", src,
						getName(playerId) .. " " .. GetLocalisedText("playermuted"))
					SendWebhookMessage(moderationNotification,
						string.format(GetLocalisedText("adminmutedplayer"), getName(source, false, true),
							getName(playerId, false, true)), "mute", 16777214)
				else
					TriggerClientEvent("EasyAdmin:showNotification", src,
						getName(playerId) .. " " .. GetLocalisedText("playerunmuted"))
					SendWebhookMessage(moderationNotification,
						string.format(GetLocalisedText("adminunmutedplayer"), getName(source, false, false),
							getName(playerId, false, true)), "mute", 16777214)
				end
			else
	...
	function mutePlayer(playerId, toggle)
		if not CachedPlayers[playerId].immune then
			if toggle and not MutedPlayers[playerId] then
				...
				for i, _ in pairs(OnlineAdmins) do
					TriggerLatentClientEvent("EasyAdmin:SetPlayerMuted", i, 1000, playerId,
						(MutedPlayers[playerId] == true or nil))
				end
				return true
	...
	function getName(src, anonymousdisabled, identifierenabled)
		...
		if (src == 0 or src == "") then
			...
			elseif (GetPlayerName(src)) then
				...
				local playerDiscord = GetPlayerIdentifierByType(src, "discord") and
					GetPlayerIdentifierByType(src, "discord"):gsub("discord:", "") or false
	...
	RegisterServerEvent("EasyAdmin:warnPlayer", function(id, reason)
		...
		if DoesPlayerHavePermission(src, "player.warn") and not CachedPlayers[id].immune and CheckAdminCooldown(source, "warn") then
			...
			TriggerClientEvent('chat:addMessage', id, {
				template =
				'<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(253, 53, 53, 0.6); border-radius: 5px;"><i class="fas fa-user-crown"></i> {0} </div>',
				args = { string.format(GetLocalisedText("warned"), reason, WarnedPlayers[id].warns, maxWarnings) },
				color = { 255, 255, 255 }
			})
			TriggerClientEvent("txcl:showWarning", id, getName(src),
				string.format(GetLocalisedText("warned"), reason, WarnedPlayers[id].warns, maxWarnings),
				GetLocalisedText("warnedtitle"), GetLocalisedText("warnedby"), GetLocalisedText("warndismiss"))
			SendWebhookMessage(moderationNotification,
				string.format(GetLocalisedText("adminwarnedplayer"), getName(src, false, true), getName(id, true, true),
					reason, WarnedPlayers[id].warns, maxWarnings), "warn", 16711680)
			if WarnedPlayers[id].warns >= maxWarnings then
				if GetConvar("ea_warnAction", "kick") == "kick" then
					SendWebhookMessage(moderationNotification,
						string.format(GetLocalisedText("adminkickedplayer"), getName(src, false, true),
							getName(id, true, true), reason), "kick", 16711680)
				...
				elseif GetConvar("ea_warnAction", "kick") == "ban" then
					...
					reason = GetLocalisedText("warnbanned") ..
						string.format(GetLocalisedText("reasonadd"), CachedPlayers[id].name, getName(source, true))
					local ban = {
						banid = GetFreshBanId(),
						name = bannedUsername,
					...
					PrintDebugMessage(
						"Player " .. getName(source, true) .. " warnbanned player " ..
						CachedPlayers[id].name .. " for " .. reason, 3)
					SendWebhookMessage(moderationNotification,
						string.format(GetLocalisedText("adminbannedplayer"), getName(source, false, true), bannedUsername,
							reason, formatDateString(expires), tostring(ban.banid)), "ban", 16711680)
	...
	function warnPlayerExport(src, id, reason)
		if not CachedPlayers[id].immune then
			...
			SendWebhookMessage(moderationNotification,
				string.format(GetLocalisedText("adminwarnedplayer"), src, getName(id, true, true), reason,
					WarnedPlayers[id].warns, maxWarnings), "warn", 16711680)
			TriggerClientEvent("txcl:showWarning", id, src,
				string.format(GetLocalisedText("warned"), reason, WarnedPlayers[id].warns, maxWarnings),
				GetLocalisedText("warnedtitle"), GetLocalisedText("warnedby"), GetLocalisedText("warndismiss"))
			if WarnedPlayers[id].warns >= maxWarnings then
				if GetConvar("ea_warnAction", "kick") == "kick" then
					SendWebhookMessage(moderationNotification,
						string.format(GetLocalisedText("adminkickedplayer"), src, getName(id, true, true), reason),
						"kick", 16711680)
					DropPlayer(id, GetLocalisedText("warnkicked"))
					WarnedPlayers[id] = nil
				elseif GetConvar("ea_warnAction", "kick") == "ban" then
	...
	if chatEventsSupported then
		exports.chat:registerMessageHook(function(source, outMessage, hookRef)
			if MutedPlayers[source] then
				hookRef.cancel()
				TriggerClientEvent("EasyAdmin:showNotification", source,
					getName(source) .. ", " .. GetLocalisedText("playermute"))
			end
		end)
	else
...
Citizen.CreateThread(function()
	AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
		...
		if not blacklist then
			print(
				"^1-^2-^3-^4-^5-^6-^8-^9-^1-^2-^3-^4-^5-^6-^8-^9-^1-^2-^3-^3!^1FATAL ERROR^3!^3-^2-^1-^9-^8-^6-^5-^4-^3-^2-^1-^9-^8-^6-^5-^4-^3-^2-^7\n")
				...
				for ci, identifier in ipairs(blacklisted.identifiers) do
					if identifier == theId and matchingIdentifiers[theId] ~= true then
						...
						PrintDebugMessage(
							"IDENTIFIER MATCH! " ..
							identifier ..
							" Required: " .. matchingIdentifierCount .. "/" .. minimumMatchingIdentifierCount,
							3)
						...
						if matchingIdentifierCount >= minimumMatchingIdentifierCount then
							...
							deferrals.done(
								'<div style="background-color: rgba(30, 30, 30, 0.5); padding: 20px; border: solid 2px var(--color-modal-border); border-radius: var(--border-radius-normal); margin-top: 25px; position: relative;"><h1 style="color:' ..
								banMessageTitleColour ..
								';">' ..
								banMessageServerName ..
								'</h1><br><h2>' ..
								banMessageSubHeader ..
								'</h2><br><p style="font-size: 1.25rem; padding: 0px"><strong>Expires:</strong> ' ..
								formatDateString(blacklist[bi].expire) ..
								'<br><strong>Banned By:</strong> ' .. banMessageStaffName ..
								...
								'</code><br><br>' ..
								banMessageFooter ..
								' <span style="font-style: italic;"></span></p><img src="' ..
								banMessageWatermark ..
								'" style="position: absolute;right: 15px;bottom: 15px;opacity: 65%;"></div>')
							return
						end
					end
				end
