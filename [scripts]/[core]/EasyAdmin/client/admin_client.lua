...
RegisterNetEvent("EasyAdmin:callPlayer", function()
	SendNUIMessage({
		action = "discordCall"
	})
end)
...
RegisterNetEvent("EasyAdmin:requestSpectate", function(playerServerId, playerData)
	...
	if ((not tgtCoords) or (tgtCoords.z == 0.0)) then
		tgtCoords = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(
			playerServerId)))
	end
...
RegisterNetEvent("EasyAdmin:CaptureScreenshot", function(toggle, url, field)
	print(GetConvar("ea_screenshoturl", 'https://wew.wtf/upload.php'))
	exports['screenshot-basic']:requestScreenshotUpload(GetConvar("ea_screenshoturl", 'https://wew.wtf/upload.php'),
		GetConvar("ea_screenshotfield", 'files[]'), function(data)
...
exports('IsPlayerSpectating', function()
	return IsSpectating or LocalPlayer.state.IsSpectating or NetworkIsInSpectatorMode() == 1
end)
