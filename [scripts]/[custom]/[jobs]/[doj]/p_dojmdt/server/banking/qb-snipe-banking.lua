if GetResourceState('snipe-banking') ~= 'started' then return end
...
lib.callback.register('p_dojmdt/server/finances/getFinances', function(source, data)
    ...
    for i = 1, #result, 1 do
        ...
        local history = MySQL.single.await('SELECT * FROM banking_transactions WHERE unique_id = ?', {
            account.identifier
        })
        if history then
            for k, v in pairs(history) do
                financesData.transactions[#financesData.transactions + 1] = { -- BankTransaction
                    ...
                    date = os.date('%H:%M %d-%m-%Y', v.created_at),
                }
            end
        end
    end
