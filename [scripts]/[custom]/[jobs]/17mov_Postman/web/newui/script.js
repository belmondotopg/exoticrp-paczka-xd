...
function showTutorial(content) {
    $(".header .bgText").text("SAMOUCZEK")
    $(".header .text").text("Samouczek")
...
async function SetHost(name, playerId, percentage) {
    ...
    oldHostId = playerId
    let mainMenuTemplate = `<div id="myTeamHostPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/hostIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Szefa</div><div class="value">${name}</div></div></div>`

    let manageRewardTemplate = `<div id="manageRewardHostPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/hostIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Szefa</div><div class="value">${name}</div></div><input type="number" class="boxInput" id="rewardInput${playerId}" value=${percentage} lastVal=${percentage}></div>`
...
async function AddMember(name, playerId, addQuitBtn, percentage) {
    let myTeamTemplate = `<div id="myTeamPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Członka</div><div class="value">${name}</div></div></div>`

    let ManageTeamTemplate = `<div id="manageMyTeamPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Członka</div><div class="value">${name}</div></div><div onclick="kickPlayer('${playerId}')" class="kickBtn"><img src="newui/assets/kickIcon.svg"></div></div>`
    
    let manageRewardTemplate = `<div id="manageRewardPlayerId${playerId}" style="transform: translateX(-106%)"  class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Członka</div><div class="value">${name}</div></div><input type="number" class="boxInput" id="rewardInput${playerId}" value=${percentage} lastVal=${percentage}></div>`
...
async function AddNearbyPlayer(name, playerId) {
    let template = `<div style="transform: translateX(-106%)" id="nearbyPlayerId${playerId}" class="box"><div class="icon"> <img src="newui/assets/newUser.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Imię Gracza</div><div class="value">${name}</div></div><div class="inviteBtn" onclick="inviteNearbyPlayer('${playerId}')"><img src="newui/assets/inviteBtn.svg"></img></div></div>`
...
function kickPlayer(id) {
    if (PlayerOwnsTeam) {
        if (myId == id) {
            $.post(`https://${GetParentResourceName()}/notify`, JSON.stringify({ msg: "Właściciel zespołu nie może go opuścić!" }))
            return
        }
...
function startJob() {
    if (PlayerOwnsTeam || scriptIntialized) {
        ...
        $.post(`https://${GetParentResourceName()}/notify`, JSON.stringify({ msg: "Tylko właściciel grupy może rozpocząć pracę!" }))
    }
}
...
window.addEventListener('message', function (event) {
    ...
    if (action == "addNewMember") {
        ...
        $("#counter").fadeOut(250)
        $(".header .bgText").text("LOBBY LISTONOSZA")
        $(".header .text").text("LOBBY LISTONOSZA")
        $("#inviteScreen").fadeOut(250)
        $("#warningScreen").fadeOut(250)
        $("#tutorialScreen").fadeOut(250)
        ...
        $("#counterValue").text(`Zebrane listy: ${0}`)
        $("#counter").fadeIn(250)
        counterActive = true
        ...
        $("#counterValue").text(`Zebrane listy: ${event.data.value.toString()}`)
