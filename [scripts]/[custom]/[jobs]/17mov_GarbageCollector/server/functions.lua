...
RegisterServerEvent("17mov_GarbageCollector:UpdateTaskProgress", function()
    local src = source
    local xPlayer = Core.GetPlayerFromId(src)

    if not xPlayer then return end
    if xPlayer.job.name ~= "garbage" then return end
    
    exports["esx_hud"]:UpdateTaskProgress(src, "Garbage")
end)