...
function showTutorial(content) {
    $(".header .bgText").text("PORADNIK")
    $(".header .text").text("Poradnik")
...
function showInvitation(inviter) {
    showingInvite = true;
    $(".header .bgText").text("ZAPROSZENIE")
    $(".header .text").text("Zaproszenie")
    $(".mainScreen").fadeOut(250)
...
function showWarning() {
    $(".header .bgText").text("OSTRZEŻENIE")
    $(".header .text").text("Ostrzeżenie")
    $(".mainScreen").fadeOut(250)
    $("#tutorialScreen").fadeOut(250)
    $("#inviteScreen").fadeOut(250)
    $(".mainScreen").fadeOut(250)
...
async function SetHost(name, playerId, percentage) {
    ...
    oldHostId = playerId
    let mainMenuTemplate = `<div id="myTeamHostPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/hostIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Szefa</div><div class="value">${name}</div></div></div>`

    let manageRewardTemplate = `<div id="manageRewardHostPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/hostIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Szefa</div><div class="value">${name}</div></div><input type="number" class="boxInput" id="rewardInput${playerId}" value=${percentage} lastVal=${percentage}></div>`
...
async function AddMember(name, playerId, addQuitBtn, percentage) {
    let myTeamTemplate = `<div id="myTeamPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Członka</div><div class="value">${name}</div></div></div>`

    let ManageTeamTemplate = `<div id="manageMyTeamPlayerId${playerId}" style="transform: translateX(-106%)" class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Członka</div><div class="value">${name}</div></div><div onclick="kickPlayer('${playerId}')" class="kickBtn"><img src="newui/assets/kickIcon.svg"></div></div>`
    
    let manageRewardTemplate = `<div id="manageRewardPlayerId${playerId}" style="transform: translateX(-106%)"  class="box"><div class="icon"><img src="newui/assets/clientIcon.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Członka</div><div class="value">${name}</div></div><input type="number" class="boxInput" id="rewardInput${playerId}" value=${percentage} lastVal=${percentage}></div>`
...
async function AddNearbyPlayer(name, playerId) {
    let template = `<div style="transform: translateX(-106%)" id="nearbyPlayerId${playerId}" class="box"><div class="icon"> <img src="newui/assets/newUser.svg" height="50px"><img src="newui/assets/strokes.svg" height="40px"></div><div class="content"><div class="topic">Nazwa Gracza</div><div class="value">${name}</div></div><div class="inviteBtn" onclick="inviteNearbyPlayer('${playerId}')"><img src="newui/assets/inviteBtn.svg"></img></div></div>`
...
function kickPlayer(id) {
    if (PlayerOwnsTeam) {
        if (myId == id) {
            $.post(`https://${GetParentResourceName()}/notify`, JSON.stringify({ msg: "Właściciel drużyny nie może jej opuścić!" }))
            return
        }
...
async function ShowNotification(type, msg) {
    ...
    if (type == "wrong") {
        if (msg.length > 135) {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification" id="notify${thisId}"><div class="flexBox"><div class="text longNotify"><div class="wrongTittle">POWIADOMIENIE</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="wrongRdyTrack" id="track${thisId}"></div></div></div></div>`
        } else {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification" id="notify${thisId}"><div class="flexBox"><div class="icon"><img src="/web/newui/assets/xMark.svg"></div><div class="text shortNotify"><div class="wrongTittle">POWIADOMIENIE</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="wrongRdyTrack" id="track${thisId}"></div></div></div></div>`
        }
    } else {
        if (msg.length > 135) {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification " id="notify${thisId}"><div class="flexBox"><div class="text longNotify"><div class="tittle">POWIADOMIENIE</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="rdyTrack" id="track${thisId}"></div></div></div></div>`
        } else {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification" id="notify${thisId}"><div class="flexBox"><div class="icon"><img src="/web/newui/assets/check.svg"></div><div class="text shortNotify"><div class="tittle">POWIADOMIENIE</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="rdyTrack" id="track${thisId}"></div></div></div></div>`
...
function startJob() {
    if (PlayerOwnsTeam || scriptIntialized) {
        ...
        $.post(`https://${GetParentResourceName()}/notify`, JSON.stringify({ msg: "Tylko właściciel drużyny może rozpocząć pracę!" }))
    }
}
...
window.addEventListener('message', function (event) {
    ...
    if (action == "addNewMember") {
        ...
        $("#counter").fadeOut(250)
        $(".header .bgText").text("ROZPOCZNIJ PRACĘ")
        $(".header .text").text("ROZPOCZNIJ PRACĘ")
        $("#inviteScreen").fadeOut(250)
        $("#warningScreen").fadeOut(250)
        $("#tutorialScreen").fadeOut(250)
        ...
        $("#counterValue").text(`Zebrane śmieci: ${0}%`)
        $("#counter").fadeIn(250)
        counterActive = true
        ...
        $("#counterValue").text(`Zebrane śmieci: ${event.data.value.toString()}%`)
