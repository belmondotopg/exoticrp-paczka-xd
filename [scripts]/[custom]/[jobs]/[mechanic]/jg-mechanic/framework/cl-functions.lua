...
function Framework.Client.ProgressBar(text, duration, anim, prop, cb, cbCancelled)
  ...
  elseif Config.ProgressBar == "auto" or Config.ProgressBar == "ox-bar" or Config.ProgressBar == "ox-circle" then
    if (Config.ProgressBar == "ox-bar" and exports.esx_hud.progressBar or lib.progressCircle)({
      duration = duration or 5000,
      label = text or "",
      useWhileDead = false,
      canCancel = true,
...
function Framework.Client.GetVehicleValue(model)
  ...
  if GetResourceState("uz_Dealerships") == "started" then
    local vehicleData = exports['uz_Dealerships']:GetVehicleData(model)
    local price = vehicleData and vehicleData.price
    local handlingPrice = GetVehicleModelValue(hash)
    if price then 
      vehicleValue = price
    elseif handlingPrice then
      vehicleValue = handlingPrice
    end
  end

  return vehicleValue or 50000
end
