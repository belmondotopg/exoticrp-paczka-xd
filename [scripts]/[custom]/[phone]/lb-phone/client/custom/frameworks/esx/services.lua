...
function GetCompanyData()
    ...
    if not companyData.isBoss then
        local permissionPromise = promise.new()
        
        ESX.TriggerServerCallback("esx_society:checkBossPermission", function(hasPermission)
            permissionPromise:resolve(hasPermission)
        end, companyData.job)
        
        local hasPermission = Citizen.Await(permissionPromise)
        
        if hasPermission then
            companyData.isBoss = true
        else
            return companyData
        end
    end
    ...
    ESX.TriggerServerCallback("esx_society:getEmployees", function(employees)
        for i = 1, #employees do
            ...
            employees[i] = {
                ...
                canInteract = employee.job.grade_name ~= "boss" and employee.identifier ~= ESX.PlayerData.identifier
            }
        end
...
function HireEmployee(source, cb)
    ...
    end, player.identifier, ESX.PlayerData.job.name, 1, "hire")
