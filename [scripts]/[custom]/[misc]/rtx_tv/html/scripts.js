...
window.addEventListener('message', function (event) {
	...
	if (item.message == "tvshow") {
		...
		document.getElementById("currentlytexttype").innerHTML= item.tvurltypedata;
		volumeslider.value = item.tvvolumedata;		
		if (item.tvurldata == "") {	
			document.getElementById("currentlytextdata2").innerHTML = "";
			document.getElementById("currently-playing").innerHTML = "<i class='fas fa-play-circle'></i> &ensp;Obecnie nie odtwarzane jest żadne wideo";
		} else {
			document.getElementById("currently-playing").innerHTML = "<i class='fas fa-play-circle'></i> &ensp;Odtwarzane obecnie";
			document.getElementById("currentlytextdata2").innerHTML= item.tvurldata;
