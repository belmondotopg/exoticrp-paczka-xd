...
function getMutedPerson(source)
	return mutedPlayers[source]
end


exports('getMutedPerson', getMutedPerson)
...
RegisterCommand('vol', function(_, args)
	...
	if not args[2] then
		TriggerEvent("esx:showNotification", 'Podaj typ voice w którym chcesz zmienić głośność, prawidłowe: ~w~radio, phone')
		return
	end	
	local volume = tonumber(args[1])
	local typee = args[2]
	if typee ~= "radio" and typee ~= "phone" then
		TriggerEvent("esx:showNotification", 'Zły typ voice! Prawidłowe: ~w~radio, phone')
		return
	end	
	if volume then
		setVolume(volume, typee)
	end
end)



RegisterNetEvent('pma-voice:ToggleRadioEffect', function(state)
	tglRadioEffect = state
	if state == true then
		micClicks = 'true'
	else
		micClicks = 'false'
...
function playMicClicks(clickType)
	if clickType then
		TriggerServerEvent("interact-sound_SV:PlayOnSource", "mic_click_on", 0.25)
	else
		TriggerServerEvent("interact-sound_SV:PlayOnSource", "mic_click_off", 0.25)
	end
end
